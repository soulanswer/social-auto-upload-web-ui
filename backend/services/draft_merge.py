...
def build_platform_kwargs(merged, common, account):
    ...
    return {
        ...
        'recommend': bool(merged.get('recommend', False)),
        ...
        'enable_generate_image': bool(merged.get('enableGenerateImage', True)),
        'collection_id': merged.get('collection', '') or '',
        'extend_link': bool(merged.get('extendLink', False)),
        'extend_link_url': merged.get('extendLinkUrl', '') or '',
        'vivo_location_name': merged.get('vivoLocationName', '') or '',
        'vivo_distribution': bool(merged.get('vivoDistribution', False)),
        'vivo_declaration': merged.get('vivoDeclaration', '') or '',
        'vivo_privacy': merged.get('vivoPrivacy', '公开') or '公开',
        'vivo_download_permission': merged.get('vivoDownloadPermission', '允许') or '允许',
    }
